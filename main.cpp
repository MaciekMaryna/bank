...
class Person 
{
    ...
    friend std::ostream & operator<<(const Person &, const Person); // Cały czas coś robię źle. Mimo przyjaźni, nie mogę sięgać do pól private. Dlatego obecnie są one w sekcji public
};
...
{
    Os  << std::endl << "\t\tFIRST NAME:    " << Prs.FirstName 
        << std::endl << "\t\tLAST NAME:     " << Prs.LastName 
...
int main()
{
    ...
    Clients[0].FirstName = "Mayer";
    Clients[0].LastName = "Jane";
